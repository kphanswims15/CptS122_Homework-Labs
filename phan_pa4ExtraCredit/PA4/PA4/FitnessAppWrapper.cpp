...
void FitnessAppWrapper::perform_user_selection(const int &option)
{
	...
	case 2: // load weekly EP
		...
		cout << "Excercise plans loaded from exercisePlans.txt!" << endl;
		break;
	...
	case 4: // store weekly EP
		...
		cout << "Excercise plans stored to exercisePlans.txt!" << endl;
		break;
...
{
	// We want to read from the begining of the file
	// We will close the stream associated with the "exercise.txt" file if it's open.
	if (this->mFstrEP.is_open()) // is_open () returns true if the file is open, false otherwise.
	{
		this->mFstrEP.close(); // close if so we can start from the beginning of the file.
	}
	else
	{
		this->mFstrEP.open("exercisePlans.txt"); // opens in the input/ouput mode by default with fstream object
...
{
	this->displayWeeklyPlan(this->mEPList); // display a weekly excercise plan to the screen!
}
...
void FitnessAppWrapper::storeWeeklyExcercisePlan() // this will call the private storeWekklyPlan() function
{
	// start overwriting from the very beginning of the "exercisePlan.txt" file.
	// closes the stream associated with the "exercisePlan.txt" 
	if (this->mFstrEP.is_open())
	{
		this->mFstrEP.close(); // close it so we can start from the beginnig of the file
	}
	else // file is not open - open it and read the week of plan from it
	{
		this->mFstrEP.open("exercisePlans.txt"); // opens in input/output mose by default with fstream object
...
bool FitnessAppWrapper::editExcercisePlan() // will prompt the user for the name of the plan to edit
{
	...
	cout << "Which excercise plan would you like to edit (provide plan name)? ";
	char planName[100] = "";
	cin.ignore(); // ignore next character in stream, which should be newline
	cin.getline(planName, 100);
	string stdPlanName = planName; // convert from char * to std::string, easier to perform comparisons
...
{
	fileStream >> plan; // extract 1 excercise plan from the file through the fileStream -
						// uses overloaded file stream extraction operator -
...
{
	fileStream << plan << endl; // write 1 excercise plan to the file -
								// uses overloaded file stream insertion operator - 
								// (fstream, ExcerisePlan)
}
